...
struct IOCP_ENV
{
	CRITICAL_SECTION _cs;		//SOCKET SET 동기화를 위한 크리티컬 섹션
	SOCK_SET		 _set;		//Accept된 SOCKET을 관리하기 위한 SET
...
void _tmain()
{
	...
	//IOCP_ENV 구조체 초기화, IOCP 생성
	IOCP_ENV ie;
	InitializeCriticalSection(&ie._cs);
	ie._iocp = CreateIoCompletionPort(INVALID_HANDLE_VALUE, NULL, 0, numOfCPU);
	...
	for (DWORD i = 0; i < numOfThread; i++)
	{
		...
		//hThreads[i] = (HANDLE)_begintrheadex(NULL, 0, IocpSockRecvProc, &ie, 0, &dwThreadId);
	}
